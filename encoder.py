...
class BEVEmbedding_y(nn.Module):
    def __init__(
        self,
        dim: int,
        sigma: int,
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.cva_s = CrossViewAttention(28, 60, 64, 64, **cross_view)
        self.layer_s = nn.Sequential(*[ResNetBottleNeck(64) for _ in range(num_layers)])

        self.bev_embedding = BEVEmbedding(dim, **bev_embedding)
        self.cross_views = nn.ModuleList(cross_views)
        self.layers = nn.ModuleList(layers)
        
        self.bev_embedding_y = BEVEmbedding_y(64, **bev_embedding)
        
        #self.cross_views_s = nn.ModuleList(cva_s)
    ...
    def forward(self, batch):
        ...
        features_s = self.spatial(self.norm(image))
        ...
        y = self.bev_embedding_y.get_prior()              # d H W
        y = repeat(y, '... -> b ...', b=b)              # b d H W
        features_s = rearrange(features_s, '(b n) ... -> b n ...', b=b, n=n)
        y = self.cva_s(y, self.bev_embedding_y, features_s, I_inv, E_inv)
        y = self.layer_s(y)
