...
class EfficientNetExtractor(torch.nn.Module):
    ...
    Sample usage:
        ...
        backbone(x)
    """
    def __init__(self, layer_names, image_height, image_width, model_name='resnet18'):
        super().__init__()
...
if __name__ == '__main__':
    ...
    for model_name in ['van-b1', 'resnet18']:
        net = resnet18()#van-b1(pretrained=True)EfficientNet.from_pretrained(model_name)
        net = net.to(device)
        #net.set_swish(False)
