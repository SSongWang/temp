# -*- coding: utf-8 -*-
"""
Created on Wed Feb 21 19:24:18 2024
...
class EfficientNetExtractor(torch.nn.Module):
    ...
    Sample usage:
        ...
        backbone(x)
    """
    def __init__(self, layer_names, image_height, image_width, model_name='van-b1'):
        super().__init__()
        ...
        # net.set_swish(False)
        self.net = van_b1(pretrained=True)
...
if __name__ == '__main__':
    ...
    for model_name in ['van-b0', 'van-b1']:
        net = van_b1(pretrained=True)#EfficientNet.from_pretrained(model_name)
        net = net.to(device)
        #net.set_swish(False)
